#!/usr/bin/env python3
"""
Parser Debug Test - Includes massive payload test case
"""
...
# ============================================================================
# TEST 7: Massive Over-Escaped Payload (The User's Probe)
# ============================================================================
# NOTE: We use a raw string r'' to simulate the input exactly as the user provided, 
# preserving literal \n characters.
test7_massive = r"""```json:function\n{\n  \"name\": \"write_file\",\n  \"args\": {\n    \"filename\": \"market_strategy.md\",\n    \"content\": \"# Market Strategy for Solid State Battery (SSB) Technology\\n\\n## 1. Summary...\\n**Key Advancements:**\\n*   **Design & Performance:** SSBs offer reduced weight...\\n\"\n  }\n}\n```"""

print("=" * 80)
print("TEST 7: Massive Over-Escaped Payload")
print("=" * 80)
print("INPUT (truncated):")
print(test7_massive[:150] + "...")
print("\nRESULT:")
blocks = extract_all_code_blocks(test7_massive, 'function')
print(f"Found {len(blocks)} blocks")
for i, block in enumerate(blocks):
    print(f"Block {i}: {block.data.get('name')} - filename: {block.data.get('args', {}).get('filename')}")
print()
...
print("=" * 80)
if len(blocks) == 1:
    print("SUCCESS: Massive payload parsed.")
else:
    print("FAILURE: Massive payload not detected.")